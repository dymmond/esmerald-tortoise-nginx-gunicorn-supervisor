...
bind = None
bind_env = os.getenv("BIND", "unix:///var/www/{{ cookiecutter.project_name }}.sock")
if bind_env:
    bind = bind_env
else:
    bind = f"{host}:{port}"
...
user = os.getenv("USER", "{{ cookiecutter.project_name }}")
group = os.getenv("GROUP", "{{ cookiecutter.project_name }}")
